...
func (t GrafanaClient) GetDashboard(DSN string) bool {
...
func (t GrafanaClient) Exec(DSN string, sql string) (int64, error) {
	//DSN = "root:taosdata@http(localhost:6041)/"
	taos, err := db.Open("taosRestful", DSN)
	if err != nil {
...
func (t GrafanaClient) BatchExec(DSN string, sqlBatch string) (int64, error) {
	taos, err := db.Open("taosRestful", DSN)
	if err != nil {
		logs.Errorf("failed to initialize connection to TDengine, err: %v", err)
		return 0, err
...
func (t GrafanaClient) Query(DSN string, sql string) (string, error) {
