...
func (a *WmsStocktakingService) AddStocktaking(ctx context.Context, req *freezonex_openiiot_api.AddStocktakingRequest, c *app.RequestContext) (*freezonex_openiiot_api.AddStocktakingResponse, error) {
	...
	for _, b := range req.ShelfRecords {
		for _, d := range b.Inventory {
			...
			data, err := tx.Find()
			_, err = stocktakingrecordservie.AddStocktakingRecordDB(ctx, common.GetUUID(), stocktakingID, common.StringToInt64(b.StorageLocationId), common.StringToInt64(d.MaterialId), d.Quantity, data[0].Quantity, d.Discrepancy)
			if err != nil {
				return nil, err
			}
		}
