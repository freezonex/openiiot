...
func (a *TenantHandler) UpdateTenantComponent(ctx context.Context, c *app.RequestContext) middleware.HandlerResponse {
	req := ctx.Value(middleware.REQUEST).(*freezonex_openiiot_api.UpdateTenantComponentRequest)
	resp, err := a.tenantService.UpdateTenantComponent(ctx, req, c)
	if err != nil {
		logs.CtxErrorf(ctx, "event=UpdateTenantComponent error=%v", err)
		return middleware.ErrorResp(http.StatusInternalServerError, err)
	}
	return resp
}
