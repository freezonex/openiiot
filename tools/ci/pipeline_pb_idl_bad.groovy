                ...
                script {
                    if (CHANGE_LIST_ID != '') {
                        withCredentials([string(credentialsId: 'hongzhi_token', variable: 'GITHUB_TOKEN')]) {
                            dir('openiiot') {
                                sh "git config user.email 'zhanghongzhi@freezonex.io'"
                                sh "git config user.name 'hongzhi1234'"
                                sh "git remote set-url origin https://hongzhi1234:\$GITHUB_TOKEN@github.com/hongzhi1234/openiiot.git"
                                sh "git add biz/model pb_idl"
                                sh "git commit -m 'Update files created by hz tool'"
                                sh "git push origin --delete ${BRANCH_NAME}"
                                sh "git push origin ${BRANCH_NAME}"
                                sh """
                                    curl -X POST -H 'Authorization: token \$GITHUB_TOKEN' \
                                    -d '{\\"title\\": \\"pb_idl update\\", \\"body\\": \\"Update auto-generated files for changelist: ${CHANGE_LIST_ID}\\", \\"head\\": \\"${BRANCH_NAME}\\", \\"base\\": \\"main\\"}' \
