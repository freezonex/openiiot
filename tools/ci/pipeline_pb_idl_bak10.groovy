                    ...
                    if (CHANGE_LIST_ID != '') {
                        ...
                        withCredentials([string(credentialsId: 'hongzhi_token', variable: 'GITHUB_TOKEN')]) {
                            dir('openiiot') {
                                ...
                                def prResponse = sh(script: "curl -s -X POST -H 'Authorization: token ${GITHUB_TOKEN}' -d '{\"title\": \"${PR_TITLE}\", \"body\": \"${PR_BODY}\", \"head\": \"${BRANCH_NAME}\", \"base\": \"main\"}' https://api.github.com/repos/hongzhi1234/openiiot/pulls", returnStdout: true).trim()
                                ...
                                if (prUrl) {
                                    ...
                                    //mail to: '4qBZkwZN5eeJ0@freezonex.io',
